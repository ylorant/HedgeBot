...
class PluginManager extends EventManager
{
        ...
        foreach ($this->_pluginsDefinition as $pluginName => $pluginData) {
            if ($pluginName != $plugin) {
                ...
                if (is_array($dependencies) && in_array($plugin, $dependencies)) {
                    HedgeBot::message('Plugin $0 depends on plugin $1. Cannot unload plugin $1.',
                        array($pluginName, $plugin), E_WARNING);
                    return false;
                }
            }
        }
    ...
    {
        HedgeBot::message('Adding routine $0, executed every $1s', array(get_class($plugin) . '::' . $method, $time),
            E_DEBUG);
    ...
    {
        HedgeBot::message('Changing routine $0 time interval to $1s', array(get_class($plugin) . '::' . $method, $time),
            E_DEBUG);
