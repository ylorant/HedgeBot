...
class Horaro
{
    ...
    public function asyncListen()
    {
        ...
        foreach($this->asyncRequests as $index => $request)
        {
            if($request['curl'] == $curlInfo['handle'])
            {
                ...
                {
                    $this->queryAsync(self::HORARO_HOST. $data->links[0]->uri, [], $request['cb'], $request['cbParams']);
                    break;
                }
        ...
        if(!is_null($foundIndex))
            ...
            HedgeBot::message("A request was completed but it looks like it doesn't belong to any pending query.", [], E_DEBUG);
    }
    ...
    {
        if(!empty($eventId))
            $this->queryAsync("/events/". $eventId. "/schedules/". $scheduleId, [], $callback, [$scheduleId, $eventId]);
        else
