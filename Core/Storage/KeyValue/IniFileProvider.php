 ...
 */
class IniFileProvider extends KeyValueProvider
{
    private $dataPath; // Root storage directory
    ...
    public function connect($parameters)
    {
        ...
        HedgeBot::message('Connecting to INI file storage at path "$0"', array($location), E_DEBUG);
        ...
        if (file_exists($location)) {
            ...
            HedgeBot::message("The specified path for file data storage doesn't exist.", null, E_WARNING);
            return false;
        }

        // Resetting the data and loading everything
        $this->data = [];
        $this->loadData();
        ...
        foreach ($dirContent as $file) {
            if (is_file($dir . '/' . $file) && pathinfo($file, PATHINFO_EXTENSION) == 'ini') {
                ...
                if (!isset($this->fileLastModification[$dir . '/' . $file])
                    ...
                    $updated = true;
                    $data = $this->loadDataFile($dir . '/' . $file);
    ...
    /**
     * Loads INI data from the current path. This method will call one of the loading methods depending on
     * whether the given path is a file or a directory.
     * 
     * @return void 
     */
    public function loadData()
    {
        if(is_file($this->dataPath)) {
            $this->data = $this->loadDataFile($this->dataPath);
        } elseif(is_dir($this->dataPath)) {
            $this->data = $this->loadDataDirectory($this->dataPath);
        }
    }
     ...
     *
     * @param string $dir The path to the directory to analyze.
     * @return array The loaded data.
     */
    public function loadDataDirectory($dir)
    {
        $dirContent = scandir($dir);
        if (!$dirContent) {
            return false;
        ...
        foreach ($dirContent as $file) {
            ...
            if (is_file($dir . '/' . $file) && pathinfo($file, PATHINFO_EXTENSION) == 'ini' && $file[0] != '.') {
                $fileData = $this->loadDataFile($dir . '/' . $file);
                $this->fileLastModification[$dir . '/' . $file] = filemtime($dir . '/' . $file);
            } elseif (is_dir($dir . '/' . $file) && !in_array($file, array('.', '..')) && $file[0] != '.') {
                $fileData = $this->loadDataDirectory($dir . '/' . $file, false);
    ...
    /**
     * Writes the current data to the currently configured path. It will either write a file or a directory
     * depending on the type of path configured.
     * 
     * @return bool True if the write succeeded, false if not. 
     */
    public function writeData()
    {
        if(is_file($this->dataPath)) {
            return $this->writeDataFile($this->dataPath);
        } elseif(is_dir($this->dataPath)) {
            return $this->writeDataDirectory($this->dataPath);
        }
     ...
     * Parameters without a section will be put in a root.ini file on the conf base dir.
     * 
     * @param string $basepath The base path to store to.
     *
     * @return bool true if the writing of the data succeeded, false otherwise.
     */
    public function writeDataDirectory($basepath)
    {
        if ($this->readonly) {
            return false;
        }
        ...
        if (!$this->emptyRecursive($root)) {
            ...
            file_put_contents($basepath . '/root.ini', $iniRoot);
        }
        ...
        foreach ($sections as $name => $section) {
            ...
            if (count($sectionComponents) == 1) {
                ...
                if (!is_dir($basepath . '/' . $sectionComponents[0])) {
                    mkdir($basepath . '/' . $sectionComponents[0]);
                }
            ...
            file_put_contents($basepath . '/' . $filename, $iniSection, FILE_APPEND);
        }
    }

    /**
     * Writes the current data into the given file path.
     * 
     * @param string $file The file path to write the data to.
     * @return bool True if the write succeeded, false if not. 
     */
    public function writeDataFile($file)
    {
        if ($this->readonly) {
            return false;
        }
        ...
        $iniData = $this->generateINIStringRecursive($this->data);
        file_put_contents($file, $iniData);
     ...
     */
    private function backupData($path, $currentDir = "")
    {
        if(is_file($path)) {
            file_put_contents($path . '.backup', file_get_contents($path));
        } elseif(is_dir($path)) {
            $baseDir = $path . "/" . $currentDir;
            $backupDir = $path . "/.backup/" . $currentDir;
            ...
            foreach ($contents as $file) {
                if (!in_array($file, array('.', '..', '.backup'))) {
                    ...
                    if (is_dir($baseDir . '/' . $file)) {
                        $this->backupData($path, $currentDir . '/' . $file);
                    } else {
