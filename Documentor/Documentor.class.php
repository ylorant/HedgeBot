...
class Documentor
{
	...
	public function generate()
	{
		...
		{
            $pluginPath = self::PLUGINS_DIRECTORY. "/". $pluginDir;
            if(is_dir($pluginPath))
            {
            	HedgeBot::message("\tDiscovered plugin $0", [$pluginDir], E_DEBUG);
            	$this->pluginDocs[] = new PluginDoc($pluginDir, $pluginPath);
            }
		}
		...
		$this->executePluginDocsFunction('readPluginInfo');
    ...
    private function writeDocs()
    {
        ...
        {
            $filename = $this->outputDir. "/". $plugin->getName(). ".md";
            $doc = (string) $plugin;
