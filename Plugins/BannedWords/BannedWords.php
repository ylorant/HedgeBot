...
class BannedWords extends PluginBase
{
    ...
    public function ServerPrivmsg($command)
    {
        ...
        $message = $this->normalize($command['message']);
        $timeoutDuration = $this->getConfigParameter($command['channel'], 'timeoutDuration');
        ...
        foreach($this->globalBannedWords as $bannedWord)
        {
            ...
            {
                IRC::message($command['channel'], ".timeout ". $command['nick']. " 1");
            }
        }
        
        // Check against channel banned words
        if(!empty($this->bannedWords[$command['channel']]))
        {
            foreach($this->bannedWords[$command['channel']] as $bannedWord)
            {
                if(strpos($message, $bannedWord) !== FALSE)
                {
                    IRC::message($command['channel'], ".timeout ". $command['nick']. " 1");
